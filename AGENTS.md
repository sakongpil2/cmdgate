...
│   │   └── audit.go          # 감사 로그 기록
│   └── policy/
│       └── policy.go         # allowlist.yaml 정책 검증
├── scripts/
...
- `policy validate`는 지정한 YAML 파일을 직접 읽어 schema 검증을 수행합니다.
...
- `cmdgate policy validate <allowlist.yaml>`은 지정한 YAML 파일을 검증합니다.
- tar/gzip 압축 정책 패키지, manifest, checksum 파일은 사용하지 않습니다.
- 검증 단계:
  1. YAML 파일 읽기
  2. allowlist.yaml 파싱
  3. `version`, `mode`, 명령 ID, 명령 문자열, matcher 참조, 지원 matcher 타입 검증
  4. 검증 성공 시 `policy_validate` audit log 기록
- 검증은 설치된 `/opt/cmdgate/allowlist.yaml`을 변경하지 않습니다.
- 운영 반영은 검증 성공 후 별도의 배포 절차로 `/opt/cmdgate/allowlist.yaml`에 복사합니다.

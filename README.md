# CmdGate

CmdGate는 운영자가 사전에 허용된 명령만 위임 권한으로 실행할 수 있도록 하는 allowlist 기반 CLI 도구입니다.

- 사용자용 바이너리: `cmdgate`
- 권한 실행기: `cmdgate-exec`
- 정책 파일: `/opt/cmdgate/allowlist.yaml`
- 감사 로그: `/var/log/cmdgate/audit.log`

`cmdgate`는 사용자 입력을 받아 `sudo -n`을 통해 `cmdgate-exec`를 호출합니다. 실제 명령 검증과 실행은 `cmdgate-exec`가 수행합니다.

## 바이너리 책임

### `cmdgate`

사용자가 직접 실행하는 CLI입니다. 입력을 검증하지 않고 `cmdgate-exec`에 그대로 전달합니다.

지원 명령:

- `cmdgate run <command> [args...]`
- `cmdgate run list`
- `cmdgate policy validate --bundle <tar.gz>`
- `cmdgate policy apply --bundle <tar.gz>`

내부적으로는 다음 형태로 `cmdgate-exec`를 호출합니다.

```bash
sudo -n /opt/cmdgate/cmdgate-exec <subcommand> [args...]
```

### `cmdgate-exec`

`sudo` 권한으로만 실행되는 권한 실행기입니다. `/opt/cmdgate/allowlist.yaml`을 읽어 사용자 입력과 비교하고, 검증에 통과한 경우에만 argv 배열 방식으로 명령을 실행합니다.

지원 명령:

- `cmdgate-exec run <command> [args...]`
- `cmdgate-exec run list`
- `cmdgate-exec policy validate --bundle <tar.gz>`
- `cmdgate-exec policy apply --bundle <tar.gz>`

주요 동작:

1. `/opt/cmdgate/allowlist.yaml`을 읽어 사용자 입력과 비교합니다.
2. 명령에 matcher placeholder가 있으면 matcher 검증을 수행합니다.
3. 검증에 성공하면 `exec.Command(cmd, args...)` 형태로 실행합니다.
4. 실행 결과를 `/var/log/cmdgate/audit.log`에 기록합니다.

## 빌드

Go 1.22 이상이 필요합니다.

```bash
go build ./cmd/...
```

빌드가 완료되면 `cmdgate`와 `cmdgate-exec` 바이너리가 생성됩니다.

## 설치

`scripts/install-cmdgate.sh`를 사용해 설치합니다. 설치 스크립트는 root 권한으로 실행해야 합니다.

```bash
# 빌드한 바이너리와 정책 파일을 scripts 디렉터리에 복사
cp cmdgate cmdgate-exec configs/allowlist.yaml scripts/

# root 권한으로 설치 스크립트 실행
sudo ./scripts/install-cmdgate.sh
```

설치 스크립트는 다음 작업을 수행합니다.

1. `/opt/cmdgate` 및 `/opt/cmdgate/work` 디렉터리를 생성합니다.
2. `/var/log/cmdgate` 디렉터리를 생성합니다.
3. `cmdgate`, `cmdgate-exec`, `allowlist.yaml`을 `/opt/cmdgate`에 복사합니다.
4. 파일 권한을 설정합니다.
5. `/etc/sudoers.d/cmdgate` 파일을 생성합니다.
6. `visudo -c`로 sudoers 설정을 검증합니다.

설치 후 다음 sudoers 규칙이 적용됩니다.

```sudoers
cmdgateadm ALL=(ALL) NOPASSWD: /opt/cmdgate/cmdgate-exec *
```

`cmdgateadm` 사용자가 존재하지 않으면 설치 스크립트는 경고를 출력합니다. 관리자가 해당 사용자를 직접 생성한 뒤에야 sudoers 규칙이 실제로 동작합니다.

## 사용 예시

### 허용된 명령 목록 보기

```bash
cmdgate run list
```

### 허용된 명령 실행

```bash
cmdgate run systemctl restart kubelet
cmdgate run journalctl -u kubelet -n 50 --no-pager
```

### 정책 번들 검증

```bash
cmdgate policy validate --bundle cmdgate-policy-1.1.0.tar.gz
```

### 정책 번들 적용

```bash
cmdgate policy apply --bundle cmdgate-policy-1.1.0.tar.gz
```

## allowlist.yaml 형식

`allowlist.yaml`은 허용할 명령과 matcher를 정의합니다.

```yaml
version: 1.0.0
mode: allowlist-only

commands:
  - id: systemctl-restart-kubelet
    desc: kubelet 재시작
    cmd: "systemctl restart kubelet"

  - id: journalctl-kubelet-lines
    desc: kubelet 로그 라인 수 지정 확인
    cmd: "journalctl -u kubelet -n <number:lines> --no-pager"

  - id: dnf-install-local-rpms
    desc: 로컬 RPM 설치
    cmd: "dnf install <rpmFiles:k8s-rpms>"

matchers:
  k8s-rpms:
    type: rpmFiles
    multiple: true
    metadataNameIn:
      - kubelet
      - containerd
      - containerd.io
      - kubeadm
      - cri-tools
      - kubectl
      - kubernetes-cni

  lines:
    type: number
```

### 명령 정의

- `version`: 정책 파일의 버전입니다.
- `mode`: 정책 모드입니다. 현재 `allowlist-only`만 지원하며, `commands`에 명시된 명령만 실행할 수 있습니다.
- `commands`: 허용할 명령 목록입니다.
- `matchers`: placeholder에서 사용할 matcher 정의입니다.

### Matcher

`<type:name>` 형태의 placeholder를 사용하면 해당 위치의 인자를 matcher로 검증합니다.

#### `number`

placeholder 위치의 인자가 10진수 숫자인지 검증합니다.

```yaml
cmd: "journalctl -u kubelet -n <number:lines> --no-pager"
```

위 예시에서 `<number:lines>` 자리에는 `50`, `100` 등의 숫자만 올 수 있습니다.

#### `rpmFiles`

입력된 RPM 파일 목록의 `NAME`이 `metadataNameIn`에 모두 포함되는지 검증합니다.

```yaml
cmd: "dnf install <rpmFiles:k8s-rpms>"
```

위 예시에서는 `k8s-rpms` matcher에 정의된 RPM 이름들만 설치할 수 있습니다.

- `type`: `rpmFiles`를 지정합니다.
- `multiple`: `true`이면 여러 개의 RPM 파일을 한 번에 지정할 수 있습니다.
- `metadataNameIn`: 허용할 RPM `NAME` 목록입니다.

## 감사 로그

모든 실행 시도는 `/var/log/cmdgate/audit.log`에 기록됩니다.

로그 항목:

- `timestamp`: 이벤트 발생 시각
- `user`: 명령을 실행한 사용자
- `action`: 수행한 동작 (예: `run`, `policy_validate`, `policy_apply`)
- `command_id`: allowlist에서 매칭된 명령 ID
- `command`: 사용자가 입력한 명령
- `result`: 실행 결과 (`success` 또는 `denied`)
- `reason`: 거부된 경우 거부 사유

## 보안 고려사항

- CmdGate는 **argv 배열 방식**으로만 명령을 실행합니다. `bash -c`, `sh -c`, `eval` 등의 shell 문자열 실행은 사용하지 않습니다.
- `cmdgate-exec`에만 위임 권한 실행 로직이 있고, 실제 권한 상승은 `sudo`를 통해 이루어집니다.
- `cmdgate`는 사용자 입력을 검증하지 않으므로 반드시 `cmdgate-exec`에서 검증이 수행됩니다.
- sudoers 규칙은 `cmdgateadm` 사용자가 `/opt/cmdgate/cmdgate-exec`만 비밀번호 없이 실행할 수 있도록 제한해야 합니다.

```sudoers
cmdgateadm ALL=(ALL) NOPASSWD: /opt/cmdgate/cmdgate-exec *
```

- `/opt/cmdgate/allowlist.yaml`의 권한을 제한하여 일반 사용자가 임의로 변경하지 못하도록 합니다.
- 정책 번들 적용 시 체크섬과 manifest를 검증하고, 기존 정책 파일을 백업한 뒤 반영합니다.
